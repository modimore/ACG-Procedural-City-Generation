...
void GLCanvas::initialize(ArgParser *_args) {
  ...
  programID = LoadShaders( "../src/makecity.vertexshader",
                           "../src/makecity.fragmentshader" );
 ...
 // store the modifier keys
  ...
  if ( (action == GLFW_PRESS || action == GLFW_REPEAT) && key < 256) {
    ...
    case 'b':  case 'B':
      ...
      break;
    case ' ':
      std::cout << "Generating additional buildings..." << std::endl;
      setupVBOs();
      break;
    case 'r':  case 'R': 
      // reset system
      std::cout << "Reseting grid..." << std::endl;
      cleanupVBOs();
      Load();
      initializeVBOs();
      setupVBOs();
